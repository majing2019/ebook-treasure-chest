...
供 /healing-video 技能调用，也提供命令行入口。
使用方式：python3 .claude/healing_schema.py <命令> [参数...]
"""
...
DB_PATH = Path(__file__).parent / "healing-tracker.db"
...
FROM stories s
LEFT JOIN scripts sc ON sc.story_id = s.id
...
def add_script(story_id: int, style: str, path: str, date: str = ""):
    """记录生成的脚本。"""
    db = get_db()
    db.execute(
        "INSERT OR REPLACE INTO scripts (story_id, style, path, generated_date) "
        "VALUES (?, ?, ?, ?)",
...
def get_unprocessed_stories(book_name: str) -> list:
    """获取某本书中状态仍为 evaluated（待用户选择）的故事。"""
    db = get_db()
    rows = db.execute("""
    ...
    if cmd == "init":
        ...
        for b in books:
            ...
            for s in b["stories"]:
                s["scripts"] = [
                    dict(r) for r in db.execute(
                        "SELECT * FROM scripts WHERE story_id=?",
                        (s["id"],)).fetchall()]
        db.close()
